...
-- [[ Toolchain path ]]
--  Runtimes are managed by mise. Prepend its shims so Mason, linters and language
--  servers resolve `node`, `ruby`, etc. even when nvim is launched from a desktop
--  entry rather than an interactive shell.
...
require('lazy').setup({
  ...
  { -- Highlight, edit, and navigate code
    ...
    --
    -- The `main` branch dropped the module system (`nvim-treesitter.configs`).
    -- Parsers are installed imperatively, and highlight/indent are opted into
    -- per buffer against Neovim's built-in `vim.treesitter`.
    config = function()
      local treesitter = require 'nvim-treesitter'

      treesitter.setup {}

      -- Installs asynchronously and skips anything already present, so the
      -- first launch after adding a language is the only one that pays for it.
      treesitter.install {
        'bash',
        'c',
        'diff',
      ...
      vim.api.nvim_create_autocmd('FileType', {
        ...
        callback = function(event)
          ...
          -- for indent rules, so leave those on the built-in indent instead.
          if lang == 'ruby' then
...
-- Use vim's native wildmenu for command line completion
-- (blink.cmp handles insert mode completion)
vim.o.wildmenu = true
vim.o.wildmode = 'longest:list,full'
vim.o.wildoptions = 'pum' -- Show completions in a popup menu
